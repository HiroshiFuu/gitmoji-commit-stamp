...
- If the message box already has a `RADAR-xxxx `, `<type><emoji>:`, `<type>:` (e.g. `fix:`), `<type>(<scope>):` / `<type>[<scope>]:` (e.g. `feat(extension):`, `fix[bug]:`), or `<emoji>:` prefix, it is replaced with the newly selected type instead of stacking up
